        <div class="sticky-head trans">
            <a href="{{ route('front.index', ['lang' => App::currentLocale()]) }}" class="sticky-logo trans fx-active">
                <h1 class="headline">{{ __('front.rest') }}<span>{{ __('front.menu') }}</span></h1>
            </a>

            {{-- Cateogry Tab --}}

            @include('components.menu.menu_category_tabs')

            {{-- Search --}}
            <form action="/{{ App::getLocale('lang') }}/search/" class="search-form" method="GET">
                <input type="search" class="search trans" placeholder="{{ __('front.search_any') }}" name="search" value="{{ Route::is('front.search') ? $q : '' }}">
                <button type="submit" class="search-submit trans">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path fill="#5E625D" d="M15.5 14h-.79l-.28-.27A6.47 6.47 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5Zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14Z" />
                    </svg>
                </button>
            </form>
            {{-- Search --}}
        </div>
