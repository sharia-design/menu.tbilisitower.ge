<x-back.layout>
    ...
    <x-slot name="title">სასმლის რედაქტირება</x-slot>
    ...
    <div class="flex bg-transparent items-center justify-center">
        <div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2">
            ...
            <div class="flex mx-7 mt-5">
                <div class="flex">
                    <h1 class="text-gray-600 font-bold md:text-2xl text-xl">სასმლის რედაქტირება</h1>
                </div>
            </div>

            <form action="{{ route('admin.drink.update', $drink->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                {{-- Name --}}
                <div class="flex flex-col m-4 p-4 rounded-md border-2 hover:border-blue-300 ease-in">
                    ...
                    <div class="flex w-full justify-between gap-1 sm:flex-col">
                        ...
                        <div class="grid grid-cols-1 mt-5">
                            ...
                            <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="name_ka" id="name_ka" type="text" value="{{ $drink->name_ka }}" required placeholder="კერძის დასახელება ქართულად" />
                            @error('name_ka')
                        ...
                        <div class="grid grid-cols-1 mt-5">
                            ...
                            <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="name_ru" id="name_ru" type="text" value="{{ $drink->name_ru }}" required placeholder="На русском языке" />
                            @error('name_ru')
                            <p class="mt-1 text-red-700">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 mt-5 ">
                            ...
                            <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="name_en" id="name_en" type="text" value="{{ $drink->name_en }}" required placeholder="English name" />
                            @error('name_en')
                            <p class="mt-1 text-red-700">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex flex-col m-4 p-4 rounded-md border-2 hover:border-blue-300 ease-in">
                    ...
                    <div class="flex w-full justify-between gap-1 sm:flex-col">
                        ...
                        <div class="grid grid-cols-1 mt-5">
                            ...
                            <textarea class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="desc_ka" id="desc_ka" placeholder="წყალი, ლიმონი, ხორცი...">{{ $drink->desc_ka }}</textarea>
                            @error('desc_ka')
                            <p class="mt-1 text-red-700">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 mt-5">
                            ...
                            <textarea class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="desc_ru" id="desc_ru" placeholder="Вода, лимон, мясо...">{{ $drink->name_ka }}</textarea>
                            @error('desc_ru')
                            <p class="mt-1 text-red-700">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 mt-5">
                            ...
                            <textarea class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="desc_en" id="desc_en" placeholder="Water, lemon, meat...">{{ $drink->name_ka }}</textarea>
                            @error('desc_en')
                            <p class="mt-1 text-red-700">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="grid grid-cols-1 mt-5 mx-7">
                    ...
                    <a href="{{ asset('public/uploads/drink/' . $drink->image) }}" target="_blank">
                        <img src="{{ asset('public/uploads/drink/' . $drink->image) }}" class="float-left block my-2 h-12">
                    </a>
                ...
                <div class="grid grid-cols-1 mt-5 mx-7">
                    ...
                    <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" name="price" id="price" type="text" value="{{ $drink->price }}" required placeholder="20,40" />
                    @error('price')
                    <p class="mt-1 text-red-700">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                    <div class="grid grid-cols-1">
                        <label for="alco" class="uppercase md:text-sm text-xs text-gray-500 text-light font-semibold">ალკოჰოლი:</label>
                        <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" type="number" name="alco" min="0" id="alco" value="{{ $drink->alco }}" required placeholder="გრადუსი" />
                        @error('alco')
                        <p class="mt-1 text-red-700">{{ $message }}</p>
                        @enderror
                    </div>
