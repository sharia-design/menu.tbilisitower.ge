<x-back.layout>
    ...
    <x-slot name="title">სასმლის დამატება</x-slot>
    ...
    <div class="flex bg-transparent items-center justify-center">
        <div class="grid bg-white rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2">
            ...
            <div class="flex mx-7 mt-5">
                <div class="flex">
                    <h1 class="text-gray-600 font-bold md:text-2xl text-xl">სასმლის დამატება</h1>
                </div>
            </div>


            <form action="/admin/drinks/create" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mt-5 mx-7">
                    <div class="grid grid-cols-1">
                        ...
                        <input class="py-2 px-3 rounded-lg border-2 border-blue-300 mt-1 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent" type="number" name="alco" min="0" id="alco" value="{{ old('alco') }}" required placeholder="გრადუსი" />
                        @error('alco')
                        <p class="mt-1 text-red-700">{{ $message }}</p>
                        @enderror
                    </div>
