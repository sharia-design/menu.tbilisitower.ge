<x-back.layout>
    ...
    <x-slot name="title">სასმლის კატეგორიები</x-slot>

    <div class="flex justify-between items-center">

        <h1 class="text-3xl">სასმლის კატეგორიები</h1>

        <a href="{{ route('admin.drink-cat.create') }}"
                            ...
                            @foreach ($categories as $cat)
                                ...
                                <tr class="text-gray-700">
                                    ...
                                    <td class="px-4 py-3 border text-sm">
                                        <a href="{{ route('admin.drink-cat.edit', $cat->id) }}" 
                                            class="block transition bg-indigo-400 rounded text-center py-1.5 mb-1 text-white hover:bg-indigo-500">
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.drink-cat.delete', $cat->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
